...
def float_distance_door(array_candiate_location):
    ...
    return tuple(
        map(lambda i, j: i - j, array_candiate_location, array_nearest_point_wall)
    )


def score_valid_motion_vector_candidates(
    tuple_old_coordinates, list_array_new_locations_available
):
    tuple_gradient_unit_vector = array_compute_unit_vector_gradient_step(
        tuple_old_coordinates
    )
    dict_tuple_of_candidates = {}
    for tuple_location_available in list_array_new_locations_available:
        tuple_new_direction_available = tuple(
            map(lambda i, j: i - j, tuple_location_available, tuple_old_coordinates)
        )
...
def move_all_points_once(set_of_points, list_vectors_directions=False):
    ...
    for tuple_set_of_points in set_of_points:
        array_moved_point = array_valid_new_point_coordinates(
            copy_set_of_points, tuple_set_of_points
        )
        copy_set_of_points.discard(tuple_set_of_points)
        copy_set_of_points.add(array_moved_point)

        if list_vectors_directions:
            new_vector_set_of_points[tuple_set_of_points] = (
                array_moved_point - tuple_set_of_points
            )
