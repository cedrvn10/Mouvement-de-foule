...
class BatchJob:
    def __init__(self, moniker, main_file, aux_files=(), timestamp=None):
        ...
        self.path = os.path.join(os.getcwd(), self.subdir)
        ...
        runscript.write("%s %s setup.cpy" % (sys.executable, main_file))
        runscript.close()
...
class GridEngineJob(BatchJob):
    def submit(
        self,
        env=(("LD_LIBRARY_PATH", INHERIT), ("PYTHONPATH", INHERIT)),
        memory_megs=None,
        extra_args=(),
    ):
...
def guess_job_class():
    ...
    qstat_helplines = (
        Popen(["qstat", "--help"], stdout=PIPE, stderr=STDOUT)
        .communicate()[0]
        .split("\n")
    )
    ...
    def __str__(self):
        return "{}({})".format(
            ...
            ",".join(
                ...
                + ["{}={}".format(kw, repr(val)) for kw, val in self.kwargs.items()]
            ),
        )
