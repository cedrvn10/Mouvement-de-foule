...
def init_epidemia_grid(fenetre):
    grille_epidemie = Canvas(fenetre, width=WINDOW.int_width_crowds_screen, height=WINDOW.int_height_crowds_screen,
                             bg='grey')
...
def create_door_on_epidemia_grid(grille_epidemie):
    grille_epidemie.create_line(
        WINDOW.dict_door_coordinates['min'] * VECTORS.array_unit_vectors_tkinter[WINDOW.dict_door_coordinates['direction']][0],
        WINDOW.dict_door_coordinates['min'] * VECTORS.array_unit_vectors_tkinter[WINDOW.dict_door_coordinates['direction']][1],
        WINDOW.dict_door_coordinates['max'] * VECTORS.array_unit_vectors_tkinter[WINDOW.dict_door_coordinates['direction']][0],
        WINDOW.dict_door_coordinates['max'] * VECTORS.array_unit_vectors_tkinter[WINDOW.dict_door_coordinates['direction']][1],
        fill='white', width="8")
    return grille_epidemie
...
def create_walls_on_epidemia_grid(grille_epidemie):
    for i in range(0, len(WINDOW.dict_dict_walls_coordinates)):
        grille_epidemie.create_rectangle(WINDOW.dict_dict_walls_coordinates[i]['point1'][0],
...
def move_set_of_points(window, set_of_points, list_id_canvas_set_of_points, canvas_id_grid_simulation):
    set_of_points, set_of_vectors = move_all_points_once(set_of_points, True)

    for i in range(0, len(set_of_points)):
        canvas_id_grid_simulation.move(list_id_canvas_set_of_points[i], set_of_vectors[i][0], set_of_vectors[i][1])

    window.after(10, move_set_of_points, window, set_of_points, list_id_canvas_set_of_points, canvas_id_grid_simulation)
