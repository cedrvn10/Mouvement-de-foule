...
def create_walls_on_epidemia_grid(grille_epidemie):
    for i in range(0, len(WINDOW.walls_coordinates)):
        grille_epidemie.create_rectangle(WINDOW.walls_coordinates[i]['point1'][0],
...
def init_set_points_epidemia_grid(set_of_points, grille_epidemie):
    ...
    for point in set_of_points:
        id_individual = grille_epidemie.create_oval(point[0] - 1,
                                                    point[1] - 1,
                                                    point[0] + 1,
                                                    point[1] + 1,
                                                    fill='blue')
...
def move_set_of_points(fenetre, set_of_points, list_id_canvas_set_of_points, grille_epidemie):
    set_of_points, new_vector_set_of_points, equiv_old_new_coordinates = move_all_points_once(set_of_points, True, True)

    list_id_canvas_set_of_points = {point: list_id_canvas_set_of_points[equiv_old_new_coordinates[point]] for point in set_of_points}

    for point in set_of_points:
        grille_epidemie.move(list_id_canvas_set_of_points[point],
