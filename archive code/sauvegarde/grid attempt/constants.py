...
class VECTORS:
    unit_vectors_tkinter = {"u": np.array([0, -1]), "d": np.array([0, 1]),
...
class WINDOW:
    width = 300
    height = 300
    number_subdivision_x_axis = int(np.ceil(np.sqrt(width)))
    number_subdivision_y_axis = int(np.ceil(np.sqrt(height)))
