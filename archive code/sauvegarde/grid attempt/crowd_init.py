...
def tuple_which_are_coordinates_of_rectangle_containing_array(array_to_locate_in_grid):
    return int(np.floor(array_to_locate_in_grid[0] / WINDOW.width_crowds_screen * WINDOW.number_subdivision_x_axis)), \
           int(np.floor(array_to_locate_in_grid[1] / WINDOW.height_crowds_screen * WINDOW.number_subdivision_y_axis))
...
def bool_in_border_of_rectangle(array_location):
    ...
    return array_location[0] / WINDOW.width_crowds_screen * WINDOW.number_subdivision_x_axis == tuple_axis_point[0] \
           and tuple_axis_point[1] == array_location[1] / WINDOW.width_crowds_screen * WINDOW.number_subdivision_x_axis
...
def set_already_contain_latter_array(list_of_points, array_coordinates):  # (?)
    tuple_which_rectangle_contains_point = tuple_which_are_coordinates_of_rectangle_containing_array(array_coordinates)
    ...
    for i in range(r0, r1):
        for j in range(r0, r1):
            if tuple_which_rectangle_contains_point[0] + i >= WINDOW.number_subdivision_x_axis or \
                    tuple_which_rectangle_contains_point[1] + j >= WINDOW.number_subdivision_y_axis or \
                    tuple_which_rectangle_contains_point[0] + i < 0 or \
                    tuple_which_rectangle_contains_point[1] + i < 0:
                pass
            for array_point_into_subset in \
                    list_of_points[tuple_which_rectangle_contains_point[0] + i][
...
def init_a_new_valid_point(set_of_points):
    while True:
        ...
        tuple_rectangle_location_point = tuple_which_are_coordinates_of_rectangle_containing_array(new_point)
        if array_point_location_available(set_of_points, new_point):
            break
    set_of_points[tuple_rectangle_location_point[0]][tuple_rectangle_location_point[1]].append(new_point)
