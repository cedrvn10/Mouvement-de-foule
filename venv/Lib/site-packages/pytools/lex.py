...
class InvalidTokenError(RuntimeError):
    ...
    def __str__(self):
        return "at index %d: ...%s..." % (
            self.index,
            self.string[self.index : self.index + 20],
        )
    ...
    if isinstance(rule, tuple):
        if rule[0] == "|":
            for subrule in rule[1:]:
                length, match_obj = _matches_rule(subrule, s, start, rule_dict, debug)
                if not length:
                    continue
            ...
            my_match_length = 0
            for subrule in rule:
                length, _ = _matches_rule(subrule, s, start, rule_dict, debug)
                if not length:
                    break
...
class LexIterator:
    ...
    def is_next(self, tag, i=0):
        return self.index + i < len(self.lexed) and self.next_tag(i) is tag
    ...
    def expected(self, what_expected):
        if self.is_at_end():
            self.raise_parse_error(
                "%s expected, end of input found instead" % what_expected
            )
        else:
            self.raise_parse_error(
                "%s expected, %s found instead" % (what_expected, str(self.next_tag()))
            )
