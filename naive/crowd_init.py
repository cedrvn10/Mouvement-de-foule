...
def array_is_outside_screen(array_coordinates):
    if not array_coordinates[0] in range(0, WINDOW.int_width_crowds_screen) or \
            not array_coordinates[1] in range(0, WINDOW.int_height_crowds_screen):
        return True
    else:
        return False
...
def array_is_into_a_wall(array_coordinates):
    for i in range(0, len(WINDOW.dict_dict_walls_coordinates)):
        if array_coordinates[0] in range(WINDOW.dict_dict_walls_coordinates[i]['point1'][0],
                                         WINDOW.dict_dict_walls_coordinates[i]['point2'][0]) and \
                array_coordinates[1] in range(WINDOW.dict_dict_walls_coordinates[i]['point1'][1],
...
def init_a_new_valid_point(set_of_points):
    while True:
        new_point = array(
            [random.randint(0, WINDOW.int_width_crowds_screen), random.randint(0, WINDOW.int_height_crowds_screen)])
