...
def array_prefered_point_to_quit(array_individuals_position):
    unit_vector_door = VECTORS.array_unit_vectors_tkinter[
        WINDOW.dict_door_coordinates["direction"]
    ]
    ...
    if (
        int_not_null_coordinate > WINDOW.dict_door_coordinates["max"]
...
def score_valid_motion_vector_candidates(
    ...
    for i in range(0, len(list_array_new_locations_available)):
        ...
        if np.array_equal(array_new_direction_available, array_gradient_unit_vector):
            ...
            float_individual_to_door = float_distance_door(
                list_array_new_locations_available[i]
            )
            dict_vector_key_scored_directions[
                float_individual_to_door
            ] = list_array_new_locations_available[i]
...
def move_all_points_once(list_array_coordinates_points, list_vectors_directions=False):
    ...
    for i in range(0, len(list_array_coordinates_points)):
        array_moved_point = array_valid_new_point_coordinates(
            list_array_new_coordinates_points, list_array_new_coordinates_points[i]
        )
        if list_vectors_directions:
            list_vector_direction_points.append(
                array_moved_point - list_array_new_coordinates_points[i]
            )
        list_array_new_coordinates_points[i] = array_moved_point
    if list_vectors_directions:
        return (
            list_array_new_coordinates_points,
            list_vector_direction_points,
        )
