...
def make_unique_filesystem_object(stem, extension="", directory="", creator=None):
    """
    ...
    if creator is None:
        ...
        def default_creator(name):
            return os.fdopen(
                os.open(name, os.O_CREAT | os.O_WRONLY | os.O_EXCL, 0o444), "w"
            )
...
def open_unique_debug_file(stem, extension=""):
    ...
    :param extension: needs a leading dot.
    """
    return make_unique_filesystem_object(stem, extension, get_run_debug_directory())
...
def refdebug(
    ...
    print("-------------->")
    try:
        reflist = [x for x in gc.get_referrers(obj) if not is_excluded(x)]
