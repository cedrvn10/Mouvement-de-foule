...
class DataTable:
    ...
    def __init__(self, column_names, column_data=None):
        ...
        self.column_indices = {colname: i for i, colname in enumerate(column_names)}
    ...
    def __str__(self):
        ...
        def format_row(row):
            return "|".join(
                [str(cell).ljust(col_width) for cell, col_width in zip(row, col_widths)]
            )

        lines = [
            format_row(self.column_names),
    ...
    def filtered(self, **kwargs):
        ...
        criteria = tuple(
            (self.column_indices[key], value) for key, value in kwargs.items()
        )
    ...
    def sort(self, columns, reverse=False):
        ...
        def mykey(row):
            return tuple(row[col_index] for col_index in col_indices)
    ...
    def aggregated(self, groupby, agg_column, aggregate_func):
        ...
        return DataTable(
            [self.column_names[i] for i in gb_indices] + [agg_column], result_data
        )
    ...
    def join(self, column, other_column, other_table, outer=False):
        ...
        result_columns = (
            [self.column_names[this_key_idx]]
    ...
    def restricted(self, columns):
        ...
        return DataTable(columns, [[row[i] for i in col_indices] for row in self.data])
